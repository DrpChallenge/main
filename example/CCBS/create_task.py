...
def convert_to_xml(input_dict: dict, output_xml_dir: Path):
    root = ET.Element("root")

    id = input_dict["id"]
    map = input_dict["map"]

    for start, goal in zip(input_dict["start"], input_dict["goal"]):
        agent = ET.SubElement(root, "agent")
        agent.set("start_id", str(start))
        agent.set("goal_id", str(goal))

    
    doc = minidom.parseString(ET.tostring(root, "utf-8"))
    with open(output_xml_dir / f"task_{id}_{map}.xml", "w") as f:
...
if __name__ == "__main__":
    ...
    for instance in instances:
        convert_to_xml(instance,tasks_dir)

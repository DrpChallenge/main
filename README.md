...
- `observation (obs)`: The obs for each drone consists of two parts: `current location` and `goal position`.  They are in soft-hot representation: the length of this vector $s^i=\left[s_1^i, \ldots, s_j^i, \ldots s_{|V|}^i\right]$ equates to the total number $|V|$ of the nodes.
